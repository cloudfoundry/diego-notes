...
| ------------         | ------------         | ------------         | ------------         | ------------         |
| `i1`: `CLAIMED ORDINARY`<br/><br/>`i2`: N/A | `cell-1`: `Present`<br/><br/>`cell-2`: `N/A`, since `i2` is not running | As part of convergence, `cell-1` changes presence:<br/>`Present` => `Missing` | `i1`: `CLAIMED SUSPECT`<br/><br/> `ActualLRPInstanceChangedEvent(Instance: i1, BeforePresence: Ordinary, AfterPresence: Suspect)`<sup>[[1]](#lrp-instance-events)</sup><br/><br/>`i2`: `UNCLAIMED ORDINARY`<br/><br/> `ActualLRPInstanceCreatedEvent(Instance: i2)`<sup>[[1]](#lrp-instance-events)</sup> | When the cell fails to establish presence, the BBS marks all instances on that cell as `SUSPECT` because the cell may or may not be functioning.<br/><br/>Replacement instances are put up for auction so they can be started on other cells. |
...
| `i1`: `RUNNING SUSPECT`<br/><br/>`i2`: `UNCLAIMED ORDINARY` | `cell-1` : `Missing`<br/><br/>`cell-2`: `Present` | `cell-2` calls `ClaimActualLRP` | `i1`: `RUNNING SUSPECT`<br/><br/> `i2`: `CLAIMED ORDINARY`<br/><br/>ActualLRPInstanceChangedEvent(Instance: `i2`, BeforeState: `Unclaimed`, AfterState: `Claimed`)<br/> | As part of the normal auction process, the available `cell-2` claims the replacement ActualLRP so that the `SUSPECT` ActualLRP can eventually be shut down. In the meantime, the `SUSPECT` instance on `cell-1` continues running to maintain routability. |
| `i1`: `RUNNING SUSPECT`<br/><br/>`i2`: `CLAIMED ORDINARY` | `cell-1` : `Missing`<br/><br/>`cell-2`: `Present` | `cell-2` calls `StartActualLRP` | `i1`: `REMOVED`<br/><br/>ActualLRPRemovedEvent(Instance: `i1`, Evacuating: `nil`)<br/><br/>ActualLRPInstanceRemovedEvent(Instance: `i1`)<sup>[[1]](#lrp-instance-events)</sup><br/><br/>`i2`: `RUNNING ORDINARY`<br/><br/>ActualLRPCreatedEvent(Instance: `i2`, Evacuating: `nil`)<br/><br/>ActualLRPInstanceChangedEvent(Instance: `i1`, Beforestate: `Claimed`, Afterstate: `Running`)<sup>[[1]](#lrp-instance-events)</sup><br/><br/> | Once `cell-2` starts a replacement ActualLRP, the `SUSPECT` instance can be removed from the missing cell and avoid any routing downtime. |
...
| `i2`: `CLAIMED ORDINARY` | `cell-1` calls `CrashActualLRP` |`i1`: `REMOVED`, ActualLRPRemovedEvent(Instance: `i1`, Evacuating: `nil`) <br><br> `i2`: `CLAIMED ORDINARY` | When the cell reports the ActualLRP has crashed, there are no longer any running ActualLRPs on either cell. Because `cell-1` is still considered suspect, the BBS decide to move forward with starting the replacement instance on a healthy cell. |
...
<a name="lrp-instance-events">1: LRP instance events are only emitted on v1 of the LRP event endpoint
