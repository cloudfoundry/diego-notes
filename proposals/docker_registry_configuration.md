...
We cannot store application metadata in-memory since we may have more than one CC. The `create app` and `start app` requests to CC may reach different instances so the authentication info stored in-memory will not be unavialable for all CC instances.
...
On Diego side we shall:
* propagate the credentials to `stager` and Docker `builder`
* (`builder`) propagate the credentials to Docker golang API to fetch the private image metadata
* (`builder`) perform `docker login` with the credentials to enable `docker pull` of private image
